...
def save_representations(net, datadir, filelist, layer, first_N=None):
	...
	image_indices = list(range(first_N))
...
def output_max_patches(max_tracker, net, layer, idx_begin, idx_end, num_top, datadir, filelist, outdir, do_which):
	...
	for cc, channel_idx in enumerate(range(idx_begin, idx_end)):
		...
		if do_info:
			...
			print('# is_conv val image_idx image_class i(if is_conv) j(if is_conv) filename', file=info_file)
		...
		for max_idx_0 in range(num_top):
			...
			if do_info:
				print(1 if mt.is_conv else 0, '%.6f' % mt.max_vals[channel_idx, max_idx], end=" ", file=info_file)
				if mt.is_conv:
					print('%d %d %d %d' % tuple(mt.max_locs[channel_idx, max_idx]), end=" ", file=info_file)
				else:
					print('%d %d' % tuple(mt.max_locs[channel_idx, max_idx]), end=" ", file=info_file)
				print(filename, file=info_file)
			...
			if do_deconv or do_deconv_norm:
				...
				out_arr[:, out_ii_start:out_ii_end, out_jj_start:out_jj_end] = net.blobs['data'].diff[0, :,
				                                                               ...
				                                                               data_jj_start:data_jj_end]
				if out_arr.max() == 0:
					print('Warning: Deconv out_arr in range', out_arr.min(), 'to', out_arr.max(),
