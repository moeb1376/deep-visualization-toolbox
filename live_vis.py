...
class LiveVis(object):
	...
	def init_window(self):
		...
		for pane_name, pane_dimensions in self.settings.window_panes:
			if len(pane_dimensions) != 4:
				raise ImproperlyConfigured(
					'pane dimensions should be a tuple of length 4, but it is "%s"' % repr(pane_dimensions))
		...
		self.help_pane.data = self.help_buffer[self.help_pane.i_begin:self.help_pane.i_end,
							  self.help_pane.j_begin:self.help_pane.j_end]
	...
	def run_loop(self):
		...
		for app_name, app in self.apps.items():
			...
			self.nets_layer_info.append(app.net_layer_info)
				...
				if hasattr(self.settings, 'debug_window_panes') and self.settings.debug_window_panes:
					for pane_name, pane in self.panes.items():
						...
						line = [FormattedString('%s |' % pane_name, self.debug_pane_defaults),
								FormattedString('pos: %d,%d |' % (pane.i_begin, pane.j_begin),
												self.debug_pane_defaults),
	...
	def handle_key_post_apps(self, key):
		...
		if tag == 'quit':
			...
			if key_label is None:
				print('Got key code %d (%s), did not match any known key (masked vals tried: %s)' % (
					key, hex(key), masked_vals_pp))
			elif tag is None:
				print('Got key code %d (%s), matched key "%s", but key is not bound to any function' % (
					key, hex(key), key_label))
			else:
	...
	def draw_help(self):
		...
		lines = []
		lines.append([FormattedString('~ ~ ~ Deep Visualization Toolbox ~ ~ ~', defaults, align='center',
									  width=self.help_pane.j_size)])
		lines.append([FormattedString('', defaults)])
		lines.append([FormattedString('Base keys', defaults)])

		for tag in ('help_mode', 'freeze_cam', 'toggle_input_mode', 'static_file_increment', 'static_file_decrement',
					'stretch_mode', 'quit'):
