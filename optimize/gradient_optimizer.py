...
class FindParams(object):
	...
	def _validate_and_normalize(self):
		...
		assert isinstance(self.push_spatial, tuple) and len(
			self.push_spatial) == 2, 'push_spatial should be a length 2 tuple'
...
class GradientOptimizer(object):
	...
	def __init__(self, net, data_mean, labels=None, label_layers=None, channel_swap_to_rgb=None):
		...
		self._data_mean_rgb_img = self.data_mean[self.channel_swap_to_rgb].transpose(
	...
	def _optimize(self, params, x0):
		...
		assert len(data_shape) in (
			2, 4), 'Expected shape of length 2 (for FC) or 4 (for conv) layers but shape is %s' % repr(data_shape)
